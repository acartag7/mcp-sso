...
test("integration — runnable example rejects duplicate /mcp Origin occurrences before allowlist matching", async () => {
  ...
  // createBridgeConfig now rejects these non-origins. Force the old malformed
...
test("integration — default allowedOrigins derives the canonical issuer origin", async () => {
  ...
  try {
    ...
    assert.deepEqual(config.allowedOrigins, [BROWSER_ORIGIN], "default is a valid canonical Origin value");
    try {
      // Browser sends the canonical origin stored by the env parser and admitted
      // independently as originOf(issuer), then reaches the bearer check.
      const res = await app.inject({ method: "POST", url: "/mcp", headers: { "content-type": "application/json", origin: BROWSER_ORIGIN }, payload: init });
      assert.equal(res.statusCode, 401, "canonical issuer origin reaches the bearer check");
