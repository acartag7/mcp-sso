...
async function initiate(c: BridgeConfig, flow: ReturnType<typeof makeFlow>["flow"], query: NormRequest["query"] = authorizeQuery()): Promise<{ res: NormResponse; claims: FlowClaims; cookieValue: string }> {
  ...
  assertCookieMutationNoStore(res, "authorize");
...
test("cookie: clearing cookie uses identical attrs + Max-Age=0 (no Domain) — both profiles", async () => {
  ...
  assertCookieMutationNoStore(res, "https callback clear");
  ...
  assertCookieMutationNoStore(resL, "loopback callback clear");
...
test("callback row 1: duplicate state/code/error/error_description => direct 400 duplicate_params (fires before cookie check)", async () => {
  ...
  const withCookie = await flow.handleCallback(callbackReq(c, "readable", { state: ["a", "b"], code: "x" }));
  assertCookieMutationNoStore(withCookie, "duplicate-parameter early return with cookie");
...
test("callback row 3: flow JWT signature/iss/aud invalid => direct 400 flow_cookie_invalid", async () => {
  ...
  assertCookieMutationNoStore(res, "invalid-cookie early return");
  assert.match(res.headers["set-cookie"] ?? "", /Max-Age=0/);
});
...
test("callback row 4 (direct mint): an expired-but-validly-signed flow => flow_expired (not flow_cookie_invalid)", async () => {
  ...
  assert.equal(res.status, 400);
  assertCookieMutationNoStore(res, "expired-flow early return");
...
test("callback row 6 + replay: single-use jti — a replayed callback is 400 flow_replayed with NO second exchange", async () => {
  ...
  assert.equal(r2.status, 400);
  assertCookieMutationNoStore(r2, "replay early return");
...
test("callback row 9: no code and no error => direct 400 missing_code", async () => {
  ...
  assert.equal(res.status, 400);
  assertCookieMutationNoStore(res, "callback missing-code error");
...
test("callback rows 12/13: success => 200 consent page + identity.verify success + oauth.upstream.callback success", async () => {
  ...
  assertCookieMutationNoStore(res, "callback success");
...
test("callback: cookie cleared on every response that had a readable cookie (success + failures)", async () => {
  ...
  assertCookieMutationNoStore(r1, "state-mismatch clear");
  ...
  assertCookieMutationNoStore(r2, "invalid-cookie clear");
  void claims;
});
...
test("entra-redirect: exchange success + verify ok => {ok:true,identity}; verify failure => identity_rejected; exchange throw/non-200/missing-id_token => exchange_failed", async () => {
  ...
  assert.equal(ok.ok && ok.identity.subject, `${entraIssuer(tenantId)}|redirect-sub`);
  ...
  const noExpToken = await new SignJWT({ tid: tenantId, nonce: "N1", sub: "redirect-sub" })
    .setProtectedHeader({ alg: "RS256", typ: "JWT", kid: "k1" })
    .setIssuer(entraIssuer(tenantId)).setAudience(clientId).setIssuedAt(now).sign(privateKey);
  const noExpTransport = { async postForm(): Promise<{ status: number; text(): Promise<string> }> { return { status: 200, text: async () => JSON.stringify({ id_token: noExpToken }) }; } };
  const noExpId = createEntraRedirectIdentity({ tenantId, clientId, redirectUri }, { transport: noExpTransport, verifyKey, currentDate: new Date(NOW_MS) });
  const noExp = await noExpId.exchangeAndVerify({ code: "c", codeVerifier: "v".repeat(43), nonce: "N1" });
  assert.deepEqual(noExp, { ok: false, kind: "identity_rejected", reason: "entra_missing_exp" });
