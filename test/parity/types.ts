...
type ExclusiveField<Key extends PropertyKey, Value, Keys extends PropertyKey> =
  Record<Key, Value> & Partial<Record<Exclude<Keys, Key>, never>>;
export type BodyValue = ExclusiveField<"absent", true, "absent" | "value">
  | ExclusiveField<"value", unknown, "absent" | "value">;
type MatcherKey = "absent" | "equals" | "matches" | "contains" | "schema";
export type Matcher = string | ExclusiveField<"absent", true, MatcherKey>
  | ExclusiveField<"equals", unknown, MatcherKey> | ExclusiveField<"matches", string, MatcherKey>
  | ExclusiveField<"contains", string, MatcherKey>
  | ExclusiveField<"schema", Record<string, unknown>, MatcherKey>;
...
type IdentityResult = { ok: true; identity: { subject: string; allowedScopes?: string[]; claims?: Record<string, unknown> }; reason?: never }
  | { ok: false; reason: string; identity?: never };
type IdentityThrow = { kind: "oauth"; code: string; description: string; status: 401 | 403 }
  | { kind: "generic"; code?: never; description?: never; status?: never };
...
export interface RequestSpec {
  ...
  body?: ExclusiveField<"json", unknown, "json" | "form" | "text">
    | ExclusiveField<"form", Array<{ name: string; value: string | CaptureReference }>, "json" | "form" | "text">
    | ExclusiveField<"text", string | CaptureReference, "json" | "form" | "text">;
}
...
export interface BootThen {
  boot: { outcome: "accepted"; error?: never } | { outcome: "rejected"; error: {
...
export interface AuditAssertion {
  events: AuditEvent[]; absent: Array<Partial<AuditEvent>>;
}
export interface AuditEvent {
  occurredAt: string; event: string; status: "success" | "failure"; clientId?: string;
...
export interface CaptureSpec {
  name: string;
  source: { bodyPointer: string; header?: never; urlQuery?: never }
