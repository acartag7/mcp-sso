...
releaseTest("RM.14 dispatch cannot be crossed: an HTTPS client id never falls back to DCR", async () => {
  ...
  // Positive control: a stored opaque client MUST take the DCR lookup path.
  // Without this, an empty lookup list below could mean only that the recorder
  // is broken and the no-fallback assertion would pass vacuously.
  const reg = await bridge.handleRegister({
    query: {}, body: { redirect_uris: [OPAQUE_REDIRECT], application_type: "web" },
    headers: { "content-type": "application/json" }, ip: "127.0.0.1",
  });
  ...
  const opaqueAuthz = await bridge.handleAuthorize({
    query: {
  ...
  assert.equal(opaqueAuthz.status, 200, "the stored opaque control must reach consent");
  assert.deepEqual(clients.lookups, [opaqueId], "the lookup recorder must observe the stored opaque authorize");
  clients.lookups.length = 0;
  ...
  // store — that would be the fallback this design forbids. `rows` records only
  // writes, so the lookup recorder is the evidence for this read-only path.
  ...
  assert.deepEqual(
    ...
    `an HTTPS id reached the DCR lookup path: ${JSON.stringify(clients.lookups)}`,
  );
});
