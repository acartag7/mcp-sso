...
import { clauseSource, loadCorpus } from "./parity/corpus.ts";
...
test("loadCorpus accepts both real 8.4 profiles and unchanged structured clones", async () => {
  const fixtures = await loadCorpus();
  assert.deepEqual(fixtures.map(({ id, profile }) => [id, profile]).toSorted(), [
    [REAL_IDS[1], "host"], [REAL_IDS[0], "portable"],
  ]);
  for (const fixture of fixtures) {
    assert.equal(fixture.contract.section, "08");
    assert.equal(fixture.contract.clause, "8.4");
    assert.match(fixture.contract.quote, /array-valued `authorization` input/);
    const root = await mkdtemp(join(tmpdir(), "mcp-sso-parity-identity-clone-"));
    try {
      await writeFixture(root, fixture.id, fixture);
      assert.deepEqual(await loadCorpus(root), [fixture]);
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  }
...
test("loadCorpus rejects an id that does not match its corpus path", async () => {
  const fixture = (await loadCorpus()).find(({ profile }) => profile === "portable")!;
  await expectCloneError(fixture, /does not match path/u, (copy) => {
    copy.id = `${copy.id}-renamed`;
  });
});
...
test("loadCorpus rejects a clause whose numeric root crosses the id section", async () => {
  ...
  await expectCloneError(fixture, /id clause 8\.4 does not match contract clause 9\.2/u, (copy) => {
    copy.contract.clause = "9.2";
  });
});

test("loadCorpus rejects a stale contract quote", async () => {
...
test("loadCorpus rejects a schema-valid clause with no canonical heading", async () => {
  const fixture = await realFixture(REAL_IDS[0]);
  const pathId = "08-resource-server-verifier/8.99-no-heading";
  await expectCloneError(fixture, /contract clause 8\.99 was not found/u, (copy) => {
    copy.id = pathId;
    copy.contract.clause = "8.99";
  }, pathId);
});

test("clauseSource returns a root clause without consuming the next root heading", () => {
  const source = "# 11. Scope contract\n\nRoot-clause sentence.\n\n# 12. Next contract\n\nOther sentence.\n";
  assert.equal(clauseSource(source, "11").trim(), "Scope contract\n\nRoot-clause sentence.");
});

test("loadCorpus resolves fixtures and contracts outside the process working directory", async () => {
