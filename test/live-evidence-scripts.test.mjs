...
const RUN_PATH = join(ROOT, "scripts/live/run.sh");
const RUN = readFileSync(RUN_PATH, "utf8");
...
test("owner-run entry point documents and allowlists every provider probe", () => {
  ...
  assert.match(RUN, /probe-entra\.mjs:entra/);
  assert.match(RUN, /probe-cloudflare\.mjs:cloudflare_access/);
  assert.match(RUN, /probe-google\.mjs:google/);
  const result = spawnSync(RUN_PATH, ["scripts/live/probe-entra.mjs", "entra"], {
    cwd: ROOT, env: { PATH: process.env.PATH ?? "" }, encoding: "utf8",
  });
  assert.notEqual(result.status, 0);
  assert.match(result.stderr, /MCP_SSO_INFRA_DIR/);
});

test("runner validates every external or generated value before export", () => {
  assert.doesNotMatch(RUN, /export [A-Z0-9_]+="\$\(/, "export must not mask command-substitution failure");
  for (const name of [
    "entra_client_id", "entra_client_secret", "entra_redirect_uri", "unmapped_group_object_id_do_not_map",
    "entra_tenant_id", "group_authorization_mapping", "cf_access_issuer", "cf_access_certs_url",
...
test("live scripts contain no private infrastructure defaults", () => {
  for (const artifact of [RUN, README]) assert.doesNotMatch(artifact, /\$HOME\/project\//);
  assert.match(RUN, /MCP_SSO_INFRA_DIR:\?/);
