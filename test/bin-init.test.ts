...
test("bin init (spawn): an internet-facing starter fails BEFORE persistent state is created", async () => {
  await ensureDist();
  const base = await mkdtemp(join(tmpdir(), "mcp-sso-init-public-starter-"));
  const proj = join(base, "proj");
  const stateDir = join(base, "state");
  try {
    await spawnScaffold(proj);
