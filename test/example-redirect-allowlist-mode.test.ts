...
test("examples — every zero-setup root rejects bad mode policy before persistence", async () => {
  ...
  const roots = [
    ...
    ["gateway", (env: Record<string, string | undefined>) => buildGatewayExample(env, GATEWAY_DEPS)],
  ] as const;
  const cases = [
    { OAUTH_REDIRECT_ALLOWLIST_MODE: "Replace" },
  ...
  try {
    for (const [root, build] of roots) {
      for (const [index, policy] of cases.entries()) {
        const stateDir = join(base, `${root}-${index}`);
        await assert.rejects(
          build({ MCP_SSO_DIR: stateDir, ...policy }),
          AuthConfigError,
        );
        assert.equal(existsSync(stateDir), false, `${root} case ${index} created no state`);
      }
    }
  } finally {
    rmSync(base, { recursive: true, force: true });
