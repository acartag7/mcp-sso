...
test("stored DCR: distinct Entra sub values sharing one mutable username never share accumulated grants", async () => {
  const ctx = setup();
  const clientId = await register(ctx);
  ...
  assert.deepEqual(await grantScopes(ctx, port, alice, clientId, "mcp:write", V("alice-write")), ["mcp:read", "mcp:write"], "Alice accumulates her own prior scope");
  assert.deepEqual(await grantScopes(ctx, port, bob, clientId, "mcp:write", V("bob-write")), ["mcp:write"], "Bob does not inherit Alice's prior read grant");
});
