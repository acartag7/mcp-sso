...
import { cacheClockObservation, finiteClockSnapshot } from "../src/ports/clock.ts";
...
test("a CIMD cache observation erases a hostile clock error into a cache miss", () => {
  const observed = cacheClockObservation({ nowMs: HOSTILE });
  assert.ok(Number.isNaN(observed));
  assert.ok(
    Number.isNaN(cacheClockObservation({ nowMs: () => 253_402_300_800_000 })),
    "a safe integer after year 9999 cannot pin cache freshness forever",
  );
});
