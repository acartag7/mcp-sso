...
The corpus contains 87 frozen fixtures: 80 portable and seven host, naming §7.2, §8.1, and §8.4. Each carries a receipt and freeze-log entry. The reference runner executes portable HTTP fixtures through Fastify, Express, and Hono, and host fixtures through Fastify. These fixtures cover Authorization occurrences, bearer fields, signature and issuer checks, audience mismatches, expiry, canonical clocks, subjects, and credential markers. The receipts in §7 and §8 separately record non-HTTP clock, verifier-result, header-boundary, and shipped-composition observations. Clause presence is not complete input-class coverage: audience-shape and verification-key-cache evidence remain incomplete, and scope and discovery fixtures are still pending. No published corpus version contains this set yet.
