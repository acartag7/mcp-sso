...
The corpus contains 173 frozen fixtures: 166 portable and seven host, naming §5, §7.2, §8.1, §8.3, §8.4, and §11. Each carries a receipt and freeze-log entry. Another 13 HTTP fixtures are draft: seven portable and six host for §8.2 challenges and §9.1 discovery. Drafts are not coverage. Portable HTTP fixtures run through Fastify, Express, and Hono; boot fixtures and host fixtures run once through the canonical host. The receipts in §7, §8, and §11 separately record non-HTTP observations. Clause presence is not complete input-class coverage: audience-shape, verification-key-cache, verifier-side scope-policy, and challenge-helper option evidence remain incomplete. No published corpus version contains this set yet.
