...
The corpus contains 87 frozen fixtures: 80 portable and seven host, naming §7.2, §8.1, and §8.4. Each carries a receipt and freeze-log entry. Another 86 portable fixtures are draft: 17 HTTP fixtures for §8.3 and §8.4 scope decisions and audit, and 69 boot fixtures for the §5 and §11 scope configuration rules. Drafts are not coverage. Portable HTTP fixtures run through Fastify, Express, and Hono; portable boot fixtures and host fixtures run once through the canonical host. The receipts in §7, §8, and §11 separately record non-HTTP observations. Clause presence is not complete input-class coverage: audience-shape, verification-key-cache, and verifier-side scope-policy evidence remain incomplete, and discovery fixtures are pending. No published corpus version contains this set yet.
