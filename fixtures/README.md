...
The corpus contains 231 frozen fixtures: 218 portable and 13 host, naming §5, §7.2, §8.1, §8.2, §8.3, §8.4, §9.1, and §11. Each carries a receipt and freeze-log entry. The portable set has 149 HTTP fixtures and 69 boot fixtures. Portable HTTP fixtures run through Fastify, Express, and Hono; boot fixtures and host fixtures run once through the canonical host. The receipts in §7, §8, and §11 separately record non-HTTP observations. There are no draft fixtures. The next milestone is publishing a release containing this first verifier slice. Later bridge and identity slices remain separate. No published corpus version contains this set yet.
