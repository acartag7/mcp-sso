...
The corpus contains 195 frozen fixtures and 36 portable HTTP drafts for incoming signed scopes. The frozen set has 182 portable and 13 host fixtures, naming §5, §7.2, §8.1, §8.2, §8.3, §8.4, §9.1, and §11. Each carries a receipt and freeze-log entry. The portable set has 113 HTTP fixtures and 69 boot fixtures. Portable HTTP fixtures run through Fastify, Express, and Hono; boot fixtures and host fixtures run once through the canonical host. The receipts in §7, §8, and §11 separately record non-HTTP observations. The remaining first-verifier source step is freezing the incoming-scope drafts after their unchanged executable content passes main CI. No published corpus version contains this set yet.
