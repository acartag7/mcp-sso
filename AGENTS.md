...
CI runs typecheck, `check:lines`, test, and build on pull requests and on `main`. Pull requests also run `process-guard` (`freeze-hash`, `mixed-diff`, `stage-artifact`), which treats `docs/contracts.md` and every `docs/contracts/*.md` file as contract paths. Branch protection requires the exact contexts `typecheck · lines · test · build` and `process-guard`. CodeQL runs on pull requests and weekly but is not required. Fork pull requests run on ephemeral runners with read-only permissions and no persisted checkout credential. Direct pushes to `main` are blocked.
...
- Explain impact in plain words: who can then do what, or what breaks. The bar: "if the rate limiter throws, `POST /oauth/register` returns 503 instead of letting registrations through", not "durable anonymous operations fail closed under limiter unavailability".
