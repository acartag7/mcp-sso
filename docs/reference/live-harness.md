...
| `rehearsal.mjs` | Every probe above, in order, through `run.sh` | Writes one receipt with the runtime commit and a `PASS`, `FAIL`, or `BLOCKED` status per row. A row is `BLOCKED` only when `run.sh` refused it for an operator-armable reason (`cloudflare_access_login_required`, `google_credentials_absent`, `infrastructure_session_expired`); any other refusal or failed check is `FAIL`. Exits 0 only when every row passed on a clean tree. A row whose output contains a configured private value is failed as `private_value_in_output` and its lines are withheld. |
| `ci/` | The same rehearsal on a GitHub-hosted runner (`.github/workflows/live.yml`) | `fetch-bundle.mjs` reads the `/mcp-sso/live/*` secrets through the OIDC-assumed role into 0600 files, `mask-bundle.mjs` registers every value with the log masker, and `ci/infra/scripts/tofu-run.sh` answers `run.sh`'s stack-output calls from the bundle so `run.sh` runs unchanged. The workflow runs nightly, on demand, and on `rehearsal/*` pushes, never on pull requests, and only under the `live` environment. |
...
The executable tests are `test/live-run-script.test.mjs`, `test/live-serve-script.test.mjs`, `test/live-e2e-probe.test.mjs`, and `test/live-rehearsal.test.mjs`. The operator record is `scripts/live/README.md`. Change the harness and that record in the same pull request.
