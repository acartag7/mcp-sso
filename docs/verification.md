...
> registered-loopback any-port for omitted or native types and keeps explicit
> `"web"` exact; a patch release decision remains.
>
...
| S6b.5 | Redirect URI match | Exact match required except that a registered loopback `http` entry may vary only its port when `application_type` is omitted or exact `"native"`; scheme, host, path, and query stay exact. Explicit `"web"` stays exact. The frozen suite pins Claude Code's literal published document without `application_type`, plus declared native/web and narrow negative cases. **IMPLEMENTED; FROZEN SUITE ACTIVE (D00-4.5.2).** |
...
- **2026-08-17 loopback interoperability correction:** the 2026-08-14
  ...
  on a validated loopback `http` entry whose type is omitted or exact
  `"native"`; only the port may differ. Explicit `"web"` stays exact-match.
  A present `"native"` or `"web"` value remains validated and carried,
