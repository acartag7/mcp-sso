...
| 11 | Consent replay | Tampering | **0.3.3:** verification returns the consent JWT's validated signed `exp`; approval atomically records that exact expiry with the JTI, then takes a fresh commit snapshot and rechecks `exp` before code storage or success audit; every store sweep retains the record while `expires_at >= now`, so restart/configuration changes cannot shorten the replay signal, and an approval delayed across `exp` cannot exploit a concurrent sweep. MySQL boot additionally requires a single-column PRIMARY or UNIQUE index on `oauth_consent_jtis.jti`, so `INSERT IGNORE` cannot silently admit a replay through a malformed pre-created schema | `MemoryStore` remains process-local: process destruction or an independent replica loses its JTI map, so durable/multi-replica replay protection requires shared persistent storage. A tombstone already swept by pre-correction code cannot be reconstructed; affected upgrades rotate the consent signing secret to invalidate outstanding consent/flow JWTs |
