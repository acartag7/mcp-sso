...
> upstream redirect-leg orchestrator — with one carve-out: the per-flow audience
> binding referenced in rows 33 and 36 is **contract-only, implementation
> pending**, so "shipped" describes the orchestrator, not that binding. Threat 34 records the contract-only,
