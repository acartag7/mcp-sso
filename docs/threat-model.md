...
| 9 | Stored-mode client spoofing (claim another's redirect), including a malformed persisted `application_type` falling through to the native loopback exception | Spoofing / Elevation | Registration validates each `redirect_uri` via the global allowlist ([§10.1](./contracts/10-redirect-uri-policy.md#101-global-allowlist-stateless-dcr-mode--assertallowedredirecturi)); every authorization read parses a fresh known-field snapshot from the untrusted `ClientStore` result and recognizes only exact `native`/`web`/`machine`; `machine` is ineligible and every other runtime discriminant is `invalid_client`. The upstream leg repeats the parse at callback before JTI consumption and every IdP-error/missing-code branch, exchange, consent signing, or success audit. Entry grammar: [row 5/9 note](#rows-59--the-redirect-entry-grammar) | None (only a currently valid, already-trusted registration can select a redirect policy) |
