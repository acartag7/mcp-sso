...
| 27 | (v0.2) Non-loopback pairing binding (envelope breach) | Spoofing / Elevation | `defaultListenHost` binds console pairing to `127.0.0.1` by default (the trust envelope is "whoever reads the process's stderr IS the operator"); real-IdP paths bind `0.0.0.0`. With no production IdP configured, both runnable examples preflight the effective `HOST` before quickstart secrets or any other state side effect and admit only exact `localhost`, `127.0.0.1`, or `::1`. A non-loopback host fails boot unless the exact deliberately unsafe escape `MCP_SSO_UNSAFE_ALLOW_NON_LOOPBACK_PAIRING=true` is present; the escape emits a loud pre-state warning and does not weaken the separate loopback issuer/resource guard | An operator who sets the explicit unsafe escape or tunnels a loopback listener publicly still exposes the pairing surface + attempt budget — bounded by maxAttempts/TTL, but outside the supported envelope. Host preflight cannot detect a tunnel, port forward, or later proxy exposure; network deployments must use a real IdP |
