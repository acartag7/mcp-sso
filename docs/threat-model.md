...
> The Entra immutable stored-grant-subject amendment is recorded in row 43.
...
| 43 | An accepted Entra account without `oid` inherits another account's stored scopes after a mutable username/email is renamed or reassigned | Spoofing / Elevation | Every Entra verification path keys the identity to an exact non-blank `oid`, otherwise to the exact already-validated issuer plus `|` plus the exact non-blank signature-verified `sub`; missing/malformed immutable claims fail with `entra_no_subject`. `preferred_username`/`email` remain display data and explicit `allowMutableClaims` allowlist candidates only; they never select the grant key. Distinct accepted issuers namespace equal `sub` values, and stored-DCR integration proves distinct `sub` values sharing one mutable claim do not accumulate each other's scopes | No automatic migration: old mutable-key grants cannot be attributed safely. Existing no-`oid` users may reapprove after full login; old access/code/consent state retains its issued subject for its normal lifetime. A legacy refresh family also preserves that subject, and successful rotation renews its TTL, so it can coexist with new issuer-namespaced families until explicit revocation, replay-family revocation, or inactivity past the current TTL. Accounts with usable `oid` are unchanged |
