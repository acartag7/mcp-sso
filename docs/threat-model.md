...
| 41 | (contract-only) One issuer serves resources A and B, but a global scope catalog, unbound refresh/machine state, membership-style JWT audience check, or shared PRM lets an A grant authorize B | Elevation / Confused deputy | **NOT active at this commit.** The pending 0.4.0 contract requires one canonical resource on every grant lineage; atomic refresh comparison/copy; explicit `legacySingletonResource` attestation before legacy-null singleton binding; machine binding at provisioning; exact primitive-string `aud`; a resource-pinned `RequestAuthorizer`; per-resource scope catalogs, PRM, and challenges; and no multi-audience token | This becomes reachable only when a deployment enables the future multi-resource form; current v0.3.2 config is singleton. The live proof must exercise two real resource endpoints and cross-resource negatives before activation. Removing config is not token revocation, and re-adding the same canonical URL restores the same resource identity; operators needing a clean replacement must use a new URL and retire old state. Dynamic resources, endpoint multiplexing, multi-audience grants, and a policy engine remain out of scope |
