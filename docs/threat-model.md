...
> live verification across Cloudflare Access, Entra, and Google. The clean-`main`
> pre-release rerun remains pending. Device flow (§17.3) and the
...
> Historical live verification also exercised refresh theft detection through
> `OAuthTokenUseCase` and `StorePort.rotateRefreshToken`; the clean-`main`
> pre-release rerun is still required.
...
| 2 | Steal/replay a refresh token | Spoofing / Elevation | Rotation marks consumed; replay ⇒ family revoked; RFC 6749 §6 client binding; rotation backfill blocks poisoning | None beyond the race window (reuse revokes immediately). Rotation remains before response preparation so replay detection cannot be skipped; malformed stored scopes can therefore rotate before returning `invalid_grant`, but require corruption, migration drift, or a nonconforming custom store (§7.4 residual) |
