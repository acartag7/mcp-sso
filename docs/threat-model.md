...
| 18 | *(v0.2, provisioning + rotation shipped S3a; client_credentials grant pending S3b)* Machine-client secret theft / misuse | Spoofing / Elevation | Out-of-band provisioning only (open DCR can NEVER mint a secret-bearing client — §17.2: `token_endpoint_auth_method`≠`none` / `grant_types`∋`client_credentials` ⇒ `invalid_client_metadata`; machine clients rejected at `/oauth/authorize`); 256-bit secrets (`mcs_`+base64url(32)), SHA-256-only storage, shown once; scopes capped by per-client `allowedScopes` (fixed ⊆ catalog at provisioning); no refresh tokens; rotation with bounded grace (≤2 active secrets) | A stolen secret is valid until rotated — there is no theft *signal* (unlike refresh replay); bounded by rotation practice + audit of `oauth.token.client_credentials` (grant lands S3b) |
