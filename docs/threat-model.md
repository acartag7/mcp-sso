...
| 6 | Token substitution across resources | Elevation | Audience fail-closed ([§7.2](./contracts/07-crypto-and-token-contracts.md#72-access-token-es256-audience-bound-fail-closed)); authorization-code redemption atomically predicates consumption on the stored canonical resource and the token use-case independently rechecks the returned record before signing, refresh persistence, or success audit | A custom StorePort that ignores the additive resource predicate may burn the legitimate code on a wrong-resource attempt, but the use-case check still prevents wrong-resource token issuance; such a store is nonconforming under §12 |
