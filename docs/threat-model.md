...
| 40 | Stored-DCR cutover is followed by a rollback; the old binary writes a new authorization code or refresh family after the one-time purge marker, then the current binary accepts or accumulates it because the client ID still exists | Tampering / Elevation | **0.3.2 PENDING:** grants issued while stored-DCR mode is active bind to library generation `1`; nullable SQL columns make old-binary inserts explicitly legacy; generation is checked during atomic code consumption/refresh rotation and prior-scope reads; returned records are rechecked; stored mode refuses a store without the generation capability | Realistic rolling/rollback operational class demonstrated by Captatum's v0.20→v0.19→v0.20 sequence. It requires write access through an older trusted binary, not a remote attacker choosing a field. Existing access JWTs are stateless and remain valid only until their normal expiry; browser-held pre-cutover consent/flow state is a separate deployment cutover concern and is not claimed fixed by this row |
