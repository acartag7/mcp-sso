...
| 11 | Consent replay | Tampering | Verification returns the consent JWT's validated signed `exp`; approval atomically records that exact expiry with the JTI, then takes a fresh commit snapshot and rechecks `exp` before code storage or success audit. The JWT also carries an opaque binding to the logical store that minted it; approval rejects a token presented to an independent store before denial, JTI consumption, code storage, or success audit. SQLite persists that binding per file and MySQL shares it across connections/replicas. A clone or restore into an independent deployment requires atomic binding rotation before service. SQL migration admits the required singleton metadata schema, initializes it under concurrent first boot, completes an interrupted empty initialization, and rejects malformed durable state before other migration writes. MySQL boot additionally requires a single-column PRIMARY or UNIQUE index on `oauth_consent_jtis.jti`; its plain insert classifies only `ER_DUP_ENTRY` as replay and propagates every other database failure | `MemoryStore` remains process-local: process destruction invalidates its outstanding consent tokens and loses its JTI map. Durable rolling restarts require SQLite or MySQL; multiple replicas require one shared MySQL database. A copied store retains its source binding until the operator rotates it; rotation invalidates outstanding consent tokens on the copy. Existing consent tokens lack the store binding and are invalidated on upgrade. A tombstone already swept by pre-correction code cannot be reconstructed; affected upgrades rotate the consent signing secret to invalidate outstanding consent/flow JWTs |
