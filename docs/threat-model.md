...
| 10 | Scope escalation or token/form expansion through a scope list | Elevation / DoS | Every scope carrier that reaches a grant or token has the same 128-entry, 256-byte-token cap; `normalizeScopes` checks the catalog (unknown ⇒ reject); server-authoritative prior-scopes are derived, not client-claimed; consent shows the delta; `requireScope` runs at the RS | Scope hierarchy remains the documented final-spec gap; this flat contract does not imply hierarchy semantics |
