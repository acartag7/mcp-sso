...
| 6 | Authorization-code substitution across resources | Elevation | Authorization-code approval requires the verified consent token's resource string to equal the exact bridge configuration before any redirect or state mutation. Redemption predicates reference-store consumption on the stored resource string and independently rechecks the returned record before signing, refresh persistence, or success audit | A custom StorePort that ignores the additive redemption predicate may burn the legitimate code on a wrong-resource attempt, but the use-case check still prevents access-token issuance from that code; such a store is nonconforming under §12. This row covers authorization-code lineage only; other grant types require separate controls |
...
| 26 | (v0.2) FIFO/special-file boot/audit hang | DoS | `open(O_NOFOLLOW \| O_NONBLOCK)` + `fstat().isFile()` on quickstart reads (`secrets.json`, `.gitignore`) and, where the native no-follow flag exists, the JSONL audit sink's append open — a FIFO at the path returns immediately instead of blocking until a writer appears; non-regular files are rejected. A host without that flag drops the audit event before opening. `openSqliteStore` opens `O_RDWR` (no block) and fails closed (`SQLITE_IOERR`) on a FIFO | None — the parity rule ([§17.8](./contracts/17-v0-2-feature-contracts.md#178-quickstart-secret-persistence-auto-keygen)) keeps every state-file open non-blocking |
...
example CF branch `ensureStateDir`, and the sqlite store — meets the full bar:
`assertRealDir`, `ensureGitignore`, `0600`/`0700`, `O_NOFOLLOW` + `O_NONBLOCK`
reads. A control fixed in one path is swept into every sibling (the global
"sweep for sibling instances" rule).
