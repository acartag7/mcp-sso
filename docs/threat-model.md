...
| 23 | (v0.2) Quickstart secret-file theft | Info disclosure | **POSIX:** `0700` dir + `0600` file + `O_EXCL` create; group/other-readable file is a BOOT FAILURE. **Windows:** mode/ownership gates are skipped entirely (no DACL support, issue #219) — `O_EXCL`, symlink/junction refusal and the regular-file check still apply, a permissive inherited ACL does NOT fail boot, and quickstart warns loudly at load. `.gitignore` written into the dir | Any process running as the same OS user can read it — the OS user account is the boundary; production uses env/secret managers |
