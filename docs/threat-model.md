...
| 23 | (v0.2) Quickstart secret-file theft | Info disclosure | **POSIX:** `0700` dir + `0600` file + `O_EXCL` create; group/other-readable file is a BOOT FAILURE. **Windows:** mode/ownership gates are skipped entirely (no DACL support, issue #219) — `O_EXCL`, symlink/junction refusal, and the regular-file check still apply, and a permissive inherited ACL does NOT fail boot. The first call in each Windows Node worker/runtime instance to `loadOrCreateQuickstartSecrets`, standalone `assertRealDir`, `ensureStateDir`, or persistent `openSqliteStore` emits one shared, fixed, path-free warning; warning failure is contained and exact `:memory:` does not consume it. `.gitignore` written into the dir | **POSIX:** any process running as the same OS user can read the file; the OS user account is the boundary. **Windows:** any principal granted read access by the inherited directory/file DACL can steal the signing key and consent secret; which principals are granted is unmeasured because the library does not inspect DACLs. Production uses env/secret managers |
