...
| 12 | Identity spoofing | Spoofing | `IdentityPort` verifies the upstream credential; no/failed identity ⇒ 401 fail-closed; no passthrough. Returned shipped reason codes are allowlisted for audit and unknown custom reasons collapse to `identity_rejected`; the public description is fixed. A thrown port error cannot author a public code, description, redirect, or audit reason; only exact rejection status 401/403 survives, while every other throw maps to generic 500 | Depends on the concrete port validating iss/aud/tid. Header mode (`identityHeader`) carries a nonce residual — [see below](#row-12--header-mode-nonce-residual). The §17.11 redirect orchestrator does not (it mints its own nonce, row 31) |
