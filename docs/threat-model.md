...
| 47 | A poisoned OIDC discovery document keeps the configured `issuer` string but substitutes an attacker-controlled authorization, token, or JWKS host | Spoofing / Elevation / Info disclosure | Discover mode validates https and exact document issuer, then binds all three discovered endpoint hostnames to the configured issuer hostname before constructing the identity. It uses WHATWG canonical hostname equality, never a registrable-domain/suffix match. The exact Google issuer has one non-configurable per-field host map for its documented cross-host endpoints; every other host fails boot before a code exchange, client-secret transmission, or remote-JWKS verifier exists | Compromise of the issuer host or any explicitly pinned Google endpoint remains inside the selected upstream identity trust boundary. Manual endpoint mode may intentionally name different hosts because each URL is explicit deployer-trusted configuration; it does not inherit trust from fetched metadata |
