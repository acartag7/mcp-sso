...
| 45 | Duplicate OAuth authorize query parameters create parser-dependent authorization input | Tampering / Spoofing | Fastify, Express, and Hono reconstruct repeated query members from the raw request URL, independent of framework query-parser configuration. One pure guard over one canonical singleton-key set rejects multiplicity at each direct adapter route before `IdentityPort.verify`, then again at `Bridge.handleAuthorize`; console pairing and upstream authorize use the same guard. Rejection is direct 400 `invalid_request` with no `Location`. Direct rejection precedes identity verification, OAuth value selection, and consent/store/success-audit work; pairing rejection additionally precedes session creation, code output, verification, and hidden-field rendering. RFC 8707's repeatable `resource` is excluded and an unsupported resource set reaches `invalid_target` | A trusted proxy that selects one occurrence before forwarding erases the evidence; it must reject duplicates or preserve the raw query. Form-body and `Authorization` ambiguity remain separate endpoint-specific contracts |
