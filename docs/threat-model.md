...
| 46 | An unauthenticated caller exhausts the protected `/mcp` verifier, MCP SDK/server construction, or gateway backend by flooding the runnable composition roots | DoS | Both Fastify examples and the generated starter install the real `@fastify/rate-limit` middleware through the isolated §8.4/§15 helper. A finite per-IP `onRequest` budget (default 60/60 s) is mandatory; normal denial is 429. `skipOnError` is fixed false and custom-store failures are converted to one fixed 503, so limiter outage/malformed state never reaches bearer audit, handler construction, backend credential access, or fetch. The existing foreign-Origin gate remains earlier and does not consume the budget. Gateway POST/GET/DELETE each use the fixed group id and carry a finite route budget. Regression tests prove denial and store failure before effects; generated-source wiring is asserted. | The built-in store is per-process, per-method-route, and bounded: traffic distributed across replicas gets one budget per process, and LRU eviction can reset a source's window under very high IP cardinality. Public multi-replica deployments need the helper's conforming shared store or aggregate limiting at a trusted edge. `request.ip` is only trustworthy when `trustProxy` matches the real topology. Custom non-Fastify `/mcp` mounts own an equivalent fail-closed admission control. The private token-only stub backend is intentionally not double-limited behind the gateway's loopback/NetworkPolicy boundary. |
