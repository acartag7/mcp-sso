...
>
> **PENDING 0.4.0:** row 41 records the planned multi-resource isolation
> control. No row-41 enforcement is active at this commit.
...
| 2 | Steal/replay a refresh token | Spoofing / Elevation | Rotation marks consumed; replay ⇒ family revoked; RFC 6749 §6 client binding; rotation backfill blocks poisoning; every failure after a successful rotation compensates by revoking the whole family before the error escapes | None beyond the race window (reuse revokes immediately). Rotation remains before response preparation so replay detection cannot be skipped. A malformed row, signing failure, or other post-rotation preparation error leaves no active unreturned successor while the store is available. If the compensating store call itself fails, the endpoint still returns no token but external storage availability determines whether revocation was durably recorded (§7.4) |
...
| 24 | (v0.2) Audit-sink loss or injection | Repudiation / Tampering | JSONL sink: JSON encoding escapes newlines (no log injection); fan-out isolation (`combineAudit`); webhook https-only (raw prefix check), redirects not followed, at-most-once; reference sinks fail open. `OAuthTokenUseCase` routes token/revocation events through `writeTokenAudit`, so even a nonconforming custom sink cannot replace those OAuth outcomes | Audit writes are fail-open by design (evidence, not a gate): sink outage = lost events; webhook is at-most-once — hard-evidence deployments use file + shipper. Other use-cases still rely on the `AuditPort` contract and the shipped fail-open sinks |
...
| 40 | Stored-DCR cutover is followed by a rollback; the old binary writes a new authorization code, refresh family, or successor in a current family after the one-time purge marker, then the current binary accepts or accumulates it because the client ID still exists | Tampering / Elevation | **0.3.2:** `OAuthAuthorizationUseCase.approve` stamps library generation `1`; nullable SQL columns make old-binary inserts explicitly legacy; `consumeAuthCode` and `rotateRefreshToken` check generation inside their atomic operation; `OAuthTokenUseCase` rechecks returned records; `findGrantedScopes` filters both family and token generations; `assertStoredDcrGenerationStore` refuses stored mode without the capability | Realistic rolling/rollback operational class demonstrated by a current→old→current application sequence. It requires write access through an older trusted binary, not a remote attacker choosing a field. Existing access JWTs are stateless and remain valid only until their normal expiry; browser-held pre-cutover consent/flow state is a separate deployment cutover concern and is not claimed fixed by this row |
| 41 | (contract-only) One issuer serves resources A and B, but ambiguous config, repaired raw URL syntax, extra definition fields, duplicate canonical URLs, a global scope catalog, unbound refresh/machine state, a membership-style JWT audience check, or shared PRM lets an A grant authorize B | Elevation / Confused deputy | **NOT active at this commit.** The pending 0.4.0 contract requires an immutable finite catalog with mutually exclusive singleton/multi forms, branch-specific boot validation, exactly the three `ResourceDefinition` keys, raw delimiter/syntax rejection before one canonical parser, and duplicate rejection; then one canonical resource on every grant lineage; atomic refresh comparison/copy with replay revocation preceding request-resource mismatch; explicit `legacySingletonResource` attestation before legacy-null singleton binding; the `resourceBinding: 1` store capability for multi-resource and stored-DCR accumulation; `machineClientResourceBinding: 1` for machine paths; machine binding at provisioning; exact primitive-string `aud`; a resource-pinned `RequestAuthorizer`; per-resource scope catalogs, PRM, and challenges; and no multi-audience token | This becomes reachable only when a deployment enables the future multi-resource form; current v0.3.2 config is singleton. The live proof must exercise two real resource endpoints and cross-resource negatives before activation. Removing config is not token revocation, and re-adding the same canonical URL restores the same resource identity; operators needing a clean replacement must use a new URL and retire old state. Pre-0.4 token handlers must be drained before multi-resource activation or singleton URL replacement and cannot safely roll back against a store containing resource-bound records, because they ignore the new columns. Dynamic resources, endpoint multiplexing, multi-audience grants, and a policy engine remain out of scope |
