...
| 41 | (contract-only) One issuer serves resources A and B, but ambiguous config, duplicate canonical URLs, a global scope catalog, or a request defaulting to the first entry conflates them | Elevation / Confused deputy | **NOT active at this commit.** Pending §5.1 requires an immutable finite catalog, mutually exclusive singleton/multi forms, one canonical parser, duplicate rejection, and per-resource catalogs/defaults. No dynamic resources, aliases, wildcard, or policy callback | This row currently covers catalog admission only. Request selection, exact token audience, persisted lineage, metadata routes, migration, and live cross-resource negatives must be separately contracted and implemented before multi-resource activation |
