...
| 41 | An unauthenticated caller exhausts Hono memory/CPU by making an OAuth POST parser materialize an oversized JSON, URL-encoded, or multipart body before Bridge admission control | DoS | The Hono adapter applies the pinned `hono/body-limit` middleware with a fixed 128 KiB cap before all four built-in OAuth POST handlers. A grammar precheck rejects malformed, duplicate/coalesced, conflicting, unsafe, or oversized `Content-Length`; valid declared lengths are removed from the middleware-visible request so actual bytes are still stream-counted. Missing-length and transfer-encoded streams therefore cannot bypass the cap. HTTP 413 is fixed and precedes parser, Bridge, limiter, store, and success-audit work. Caller-owned Hono POST authorize routes, including console pairing with `skipAuthorize`, reuse the exported `honoOAuthBodyLimit` before parsing. Runtime-owned raw-Request extensions survive Hono's reconstruction so `clientIp` still sees connection metadata | The middleware buffers a legitimate body up to 128 KiB and may read one host-sized crossing chunk without retaining or parsing it; it stops pulling but does not guarantee transport cancellation. Upload draining/timeouts and any lower limit belong to the host or reverse proxy. Fastify's shipped/default parser cap and Express's body-parser defaults remain composition-owned sibling controls rather than this Hono-specific middleware |
