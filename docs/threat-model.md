...
| 39 | A broken custom `ClockPort` supplies a non-finite, fractional, non-canonical, or backward-moving UTC value, disabling token expiry, breaking store timestamps, or replacing the intended OAuth failure during audit formatting | Spoofing / Elevation | `finiteClockSnapshot` validates each underlying read against the canonical four-digit UTC range. `verifyAccessToken` / `verifyConsentToken` reject before `jwtVerify`; `RequestAuthorizer.authorize` reuses one fixed snapshot throughout. `OAuthAuthorizationUseCase.approve` reuses an initial snapshot through verification and every pre-JTI-consume timestamped path, then after successful JTI consumption takes a fresh commit snapshot that validates the authorization-code TTL offset, cannot precede the initial snapshot, rechecks signed `exp`, and owns code expiry plus later audits. An invalid/backward commit snapshot emits no fabricated audit and leaves the JTI consumed; a valid snapshot at/after `exp` audits `invalid_consent` at the commit time before any code write or success audit. Authorization-code exchange, refresh rotation, and client credentials each validate one operation snapshot plus relevant token TTL offsets before grant/auth/store work, then reuse it for JWT dates, store timestamps, compensation, and audit. An invalid issuance snapshot mutates nothing and produces only the sanitized 500 channel | This is hardening, not a normal remote clock-control path: exploitation requires a faulty/custom trusted port plus an expired but otherwise valid signed token. The clock remains trusted; a plausible monotonic but stale canonical value is not detectable. Invalid snapshots become a fail-closed authentication outage; paths without an honest canonical timestamp emit no timestamped event |
