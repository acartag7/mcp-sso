...
| 43 | A browser, intermediary, or shared cache stores and later replays a credential-bearing authorization response | Info disclosure / Spoofing | Shared framework-free response/header helpers add `Cache-Control: no-store` to the consent HTML carrying the signed subject-bound consent JWT, the successful code-bearing approval 302, the upstream authorize 302 that sets the signed flow cookie, and every callback response that clears a readable flow cookie. Fastify, Express, and Hono relay those normalized headers unchanged. Regression coverage exercises the shared Bridge/flow helpers, all mutable callback exits, and real adapter response mapping | A nonconforming proxy can strip or ignore the directive. Pairing pages and generic sanitized errors carry no credential under this contract and are not included; PRM, AS metadata, and JWKS retain their intentional public caching policies |
