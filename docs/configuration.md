...
all three places that allowlist is read — the DCR registration write
(`registerClient`, `src/register.ts`), stateless authorize, and the
re-validation of an already-stored client's entries (`resolveOpaqueRedirect`,
`src/authorize-internals.ts`) — so a client registered while the built-ins were
trusted stops authorizing after the switch rather than being grandfathered in.
