...
The runnable examples enable Client ID Metadata Documents (CIMD) while retaining
Dynamic Client Registration (DCR). The Fastify/SQLite production example uses
stateless DCR by default and accepts `OAUTH_DCR_MODE=stored` to persist native
CLI registrations in its existing SQLite database. The API-key gateway example
retains stateless environment wiring. The server generated by `mcp-sso init`
enables CIMD and stores DCR registrations in its SQLite database. CIMD needs no
additional environment variable or secret: a CIMD-capable client supplies an
HTTPS URL as its `client_id`, and the bridge retrieves the client metadata
through `CimdResolver.resolve`.
...
path at runtime, so configure both loopback hosts as explicit portless origins:
...
bits do not enforce Windows access control. The first call in each Windows Node
worker/runtime instance to `loadOrCreateQuickstartSecrets`, standalone
`assertRealDir`, `ensureStateDir`, or `openSqliteStore` with a persistent path
prints one shared, fixed warning because the library did not verify that ACL;
the warning is not an admission check. Exact `:memory:` SQLite does not consume
