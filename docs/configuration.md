...
On `main`, the runnable examples (`configFromEnv`, `buildExample`, and
`buildGatewayExample`) and the server generated by `mcp-sso init` enable Client
ID Metadata Documents (CIMD) while continuing to expose stateless Dynamic
Client Registration (DCR). CIMD needs no additional environment variable or
secret: a CIMD-capable client supplies an HTTPS URL as its `client_id`, and the
bridge retrieves the client metadata through `CimdResolver.resolve`.
...
negotiate or fall back between them. CIMD remains unreleased until 0.3; npm
0.2.3 is the published baseline. See the
...
| `OAUTH_REDIRECT_ALLOWLIST` | optional | empty | Comma-separated redirect URIs for opaque DCR clients. CIMD redirect URIs come from the fetched document. |
