...
bits do not enforce Windows access control. The first call in a Windows process
to `loadOrCreateQuickstartSecrets`, `ensureStateDir`, or `openSqliteStore` with a
persistent path prints one shared, fixed warning because the library did not
verify that ACL; the warning is not an admission check. Exact `:memory:` SQLite
does not consume the one-shot warning.
