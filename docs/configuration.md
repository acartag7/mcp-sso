...
The runnable examples and the server generated by `mcp-sso init` MUST enable
Client ID Metadata Documents (CIMD) by default while continuing to expose
stateless Dynamic Client Registration (DCR) for clients that use it. CIMD needs
no additional environment variable or secret: a CIMD-capable client supplies
an HTTPS URL as its `client_id`, and the bridge retrieves the client metadata
through its guarded fetcher.
...
`cimd` keeps CIMD disabled. See
[§17.1](contracts/17-v0-2-feature-contracts.md#171-client-id-metadata-documents-cimd)
