...
2. **`subjectAllowlist` is optional defense-in-depth.** It matches the exact
   ...
   candidates are compared case-insensitively; whitespace remains significant.
   That opt-in affects allowlist matching only. The stored grant subject is the
   exact usable non-blank `oid`, or, when no usable `oid` exists, the exact accepted
   issuer + `"|"` + the exact usable non-blank signature-verified `sub`. Username
   and email never select it.
