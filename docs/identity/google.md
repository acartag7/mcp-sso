...
outside the configured Workspace. Before release, re-run the checklist at the
top of [`src/identity/google.ts`](../../src/identity/google.ts) against clean
`main`; that pre-release provider rerun remains pending.
