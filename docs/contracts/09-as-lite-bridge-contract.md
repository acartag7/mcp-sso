...
- **`refresh`**: atomically rotates the refresh token (§7.4), preserving
  ...
  **0.3.2:** `OAuthTokenUseCase.refresh` requires stored-DCR-mode refresh
  ...
  as a substitute.
- **`revoke`** (RFC 7009): before body normalization or token extraction,
  ...
  fail-open under §6.7. Once admitted, revocation **always returns 200**; an
...
- **Endpoints:** GET `/.well-known/oauth-authorization-server` →
  ...
  POST `/oauth/revoke` → `revoke` (behind `RateLimitPort("revoke:<ip>")` before
  body normalization; after admission it retains RFC 7009's always-200 behavior).
