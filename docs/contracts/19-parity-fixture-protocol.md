...
- **Fixture.** One JSON document that states, for one contract clause: the configuration and pre-state, every injected value (clock, randomness, keys, recorded outbound HTTP), one inbound HTTP request, and the exact observations that clause asserts (status, headers, body, audit events, store effects, or outbound calls). Its shape is fixed by `fixtures/schema/fixture.schema.json`.
