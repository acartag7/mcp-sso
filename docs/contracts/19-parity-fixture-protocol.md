...
- `store_instance`, singleton key: `instance_id`: `instance_id` and optional `swept_through`. Exactly zero or one such row may exist. `swept_through` carries a consent-replay sweep the row's store already performed, and the projection preserves it for later chain members.
