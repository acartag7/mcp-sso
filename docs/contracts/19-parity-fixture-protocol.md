...
- **Statement key.** An append-only identifier for one normative sentence, written as the clause plus a lowercase letter, for example `8.4.b`. The contract Markdown carries the same inline anchor immediately before the sentence. A letter is never reassigned after its sentence is removed. The fixture still quotes the complete sentence, so rewording it makes the fixture stale.
