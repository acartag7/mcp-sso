...
- **Statement key.** An append-only identifier for one normative sentence, written as the clause plus a bijective base-26 lowercase suffix, for example `8.4.b`. Suffixes continue `a` through `z`, then `aa` through `az`, then `ba`, with the grammar `[a-z]+`. The suffix order has no meaning. A key is never reassigned or changed after its sentence is removed or its clause is later split. The contract Markdown carries a raw HTML anchor in the exact form `<a id="8.4.b"></a>` immediately before the sentence on the same source line. The fixture still quotes the complete sentence, so rewording it makes the fixture stale. More than 26 anchors in one clause signals that the clause needs a later, separate contract split. That split does not renumber or replace existing anchors.
- **Normative prose.** Only Markdown prose outside CommonMark fenced code blocks carries a rule. Inline code spans remain part of the prose inventory. A fenced block is non-normative in every language. A comment inside a fence may point to its prose owner by statement key, but it does not restate the rule.
...
A fixture `id` is `<NN>-<section-slug>/<clause>-<slug>`, for example `08-resource-server-verifier/8.4-duplicate-authorization-fails-closed`. `contract.section`, `contract.clause`, and `contract.statement` name the exact statement the fixture pins. The fixture quotes the complete sentence in `contract.quote` so the coverage gate (§19.4) detects a later rewording. Draft bootstrap fixtures may omit `contract.statement` only until the statement-anchor migration lands; a frozen fixture cannot omit it.
...
**Coverage gate.** Every normative prose statement in §05–§17 (MUST, MUST NOT, never, always, cannot, fails closed) has a statement key and one coverage entry in `MANIFEST.json`. An entry is either uncovered or names one or more evidence records. There are no exemptions. The evidence kinds are `fixture` for a frozen HTTP fixture, `boot` for a frozen boot fixture, and `suite` for a passing suite receipt. Every fixture evidence record includes its id and `portable` or `host` profile. A normative sentence that lands without evidence fails the release gate the same way a release-matrix row without evidence does (`docs/verification-design.md`). CI rejects `MUST`, `MUST NOT`, `never`, `always`, `cannot`, or `fails closed` inside any fenced block, regardless of its language. There is no escape hatch. Only frozen portable HTTP and boot fixtures satisfy portable parity coverage. Host and suite evidence may satisfy their own release rule, but neither makes a shared OAuth rule look portable. The map lists every uncovered statement explicitly rather than reducing the gap to a count.
...
HTTP and boot fixtures are Tier 1 evidence in the sense of `docs/verification-design.md`. The coverage map also points to the stronger or specialized evidence below. It never marks their rules exempt:

- **Tier 2, packaging.** That the shipped artifact (npm tarball, container image, binary) contains and exports the tested behavior is proven per implementation, per artifact. Section 15 statements point to packed-artifact suite receipts.
- **Tier 3, live compatibility.** That a named client on a recorded version completes a real flow against a real provider is proven by the live client matrix for that implementation. Parity on the corpus is necessary for a compatibility claim, not sufficient. Live statements point to live-matrix suite receipts.
- **Store conformance (§12).** Store invariants stay as an executable suite per implementation, driven by the §12 tables. Their statement entries point to store-conformance suite receipts, so an implementation cannot claim section coverage while skipping a store row.
