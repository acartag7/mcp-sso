...
- **Fixture.** One JSON document that states one contract rule and its exact observations. An HTTP fixture composes the app and sends one inbound request, whose method is one of `GET`, `POST`, `PUT`, `HEAD`, `OPTIONS`, and `DELETE`; the schema admits no other method. A boot fixture supplies raw configuration and ports, then asserts exact startup acceptance or rejection without sending a request. Their shapes are fixed by `fixtures/schema/fixture.schema.json`.
...
- `store_instance`, singleton key: `instance_id`: `instance_id` and optional `swept_through`. Exactly zero or one such row may exist. `swept_through` carries a consent-replay sweep the row's store already performed, and the projection preserves it for later chain members. The runner seeds its consent-replay fence from the hydrated row, so a chained fixture inherits the sweep an earlier member performed.
...
- Every fixture header-map key is a lowercase HTTP field name. `then.headers` and a `given.http` request matcher assert only the names they list. A listed `{ "absent": true }` header must not exist. Each `then.outbound` entry instead lists the exact outbound header-name set, so an extra outbound header fails the fixture. A single occurrence is a string and multiple occurrences are an ordered string array. The schema bounds the vocabulary: a field name is at most 256 characters and one value occurrence at most 8192 characters, a wire header map at most 64 names, and an occurrence array at most 16 entries, so the worst valid fixture stays inside every HTTP parser's default header-count limit. Every wire header map the runner sends or renders totals at most 65536 UTF-8 bytes across names and values, counted after capture resolution; a map beyond that bound fails the fixture. Every HTTP parser the runner operates is sized above the corpus header bounds, so no schema-valid fixture is rejected for its header block before an adapter runs.
...
This section, the fixture schema, the reference TypeScript runner, and four `draft` fixtures for clause 8.4 exist. The reference runner executes every draft on every pull request: each portable draft runs through Fastify, Express, and Hono, and each host draft runs through Fastify. A draft that fails is a broken build. The host fixture (`08-resource-server-verifier/8.4-duplicate-authorization-fails-closed`) pins the reference implementation's origin-root protected-resource metadata location. The portable fixture accepts only the origin-root location or Atesaki D1's route-scoped path-inserted location while keeping the rest of the challenge exact. Neither fixture is frozen. The reference runner must pass a draft unchanged before that fixture can freeze. The record for a fixture is its `status`, its `receipt`, the clause and its quoted sentence, and `FREEZE-LOG.md`; no manifest, catalogue, or hash gate is built. The official MCP conformance suite has not yet been run against any shipped composition. Section 16 remains self-declared until it is. Until then no implementation may cite this section as evidence of parity or conformance.
