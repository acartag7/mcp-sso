...
different generation is consumed/burned and returns the same `invalid_grant` as
an unknown code. A valid stored `ClientStore` row does not make a legacy code
eligible.
