...
| Claims-only verified attributes (§17.11) | Identity completion converts optional attributes into bounded, plain, deeply frozen JSON data before `onIdentity`. It does not spread or pass the port-owned graph | A prototype, accessor, cycle, non-JSON value, or exceeded bound is row-10 `exchange_failed`. Bridge completion keeps ignoring optional attributes |
| Claims-only host response (§17.11) | The completion boundary snapshots a plain response and header record into bounded strings before success or adapter work. Header names are compared case-insensitively before projection | A prototype, accessor, duplicate header, unsafe value, or exceeded bound is row-12I `completion_failed` |
