...
| RFC 8707 audience fail-closed | ✅ v0.1; **hardened in 0.4.0** — verification requires `aud` to be a primitive string equal to the verifier's pinned resource. A multi-audience token (`aud: [A, B]`) is rejected at every resource: jose's expected-audience option uses membership semantics, so the array form satisfied `jwtVerify` before the explicit check was added | §7.2, §5.1 |
| RFC 8707 multi-resource (one issuer, N resources) | ✅ 0.4.0 — each resource has its own audience, scope catalog, consent, authorization code, refresh family, machine credentials, verifier instance, PRM document and challenge. Exactly one resource binds into every grant-bearing artifact; no grant spans resources. Cross-resource negatives (token, refresh, prior-grant evidence, metadata route collision) are covered by the suite and by a two-resource run through the official MCP SDK client. **Real-client interop is not yet verified** — see docs/live-verification.md checklist E | §5.1, §7.2, §9.1, §12 |
