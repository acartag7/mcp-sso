...
| `C` with disclosed deviation/overlay | 2 | D00-4.5.2, D00-6.5.1 | Conformant on the primary path, with a departure from literal `-00` text stated in the row rather than absorbed into the total. |
...
| D00-4.5.2 | §4.5 request redirect **MUST** exactly match a registered URI. Exact match holds for every `https` entry; the loopback any-port branch is a **profile overlay** `-00` does not state. | C for exact match; loopback any-port disclosed as an mcp-sso/MCP profile overlay | Document match: `src/cimd/registration.ts:82-95`, called at authorize (`src/authorize-internals.ts:63,68`) and callback (`src/adapters/upstream-flow-cimd.ts:80`) | Frozen `s6b-redirect.test.ts:177-217,299-308,363-385`; HTTPS is raw-exact and loopback varies only by port. **Overlay scope, stated precisely:** RFC 8252 §7.3's any-port `MUST` covers loopback **IP literals** for **native apps**, and RFC 9700 §§2.1/4.1.3 admit only that exception; mcp-sso applies any-port to `localhost` as well (`src/redirect-entry.ts:5`) and carries no native-app client type, so the RFCs alone do not justify the full matcher — it is a deliberate MCP-client-compatibility overlay. The loopback branch omits `search` comparison, but `src/redirect-entry.ts:46` rejects any raw `?` on both sides, so a differing query is unreachable rather than unchecked. **Not document enforcement:** `src/token.ts:207-218` binds the token request only to the stored authorization-code record. |
...
- **D00-4.5.2 overlay scope narrowed.** RFC 8252 §7.3 covers loopback IP
  literals for native apps; the matcher also accepts `localhost` and carries no
  native-app type, so the RFCs alone do not justify it.
