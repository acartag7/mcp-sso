...
in each Node worker/runtime instance to any of `openSqliteStore` with a
persistent path, `loadOrCreateQuickstartSecrets`, standalone `assertRealDir`,
or `ensureStateDir` — the four public filesystem-admission calls §17.8's parity
rule names, each of which skips the POSIX mode/UID gates on win32 — the shared
helper emits one fixed warning that
...
`new SqliteStore(database)` perform no library filesystem admission and emit no
