...
Every successful `IdentityPort.verify` and `RedirectIdentityPort.exchangeAndVerify` result crosses one shared subject boundary before audit, authorization, or a host callback. `IdentityClaims.subject` is a primitive string with at least one non-whitespace code point and at most 2048 UTF-8 bytes. The snapshot preserves accepted subject bytes exactly; it does not trim or normalize them. Empty, whitespace-only, over-bound, wrongly typed, or accessor-throwing subjects are malformed. The direct identity path maps them through its existing generic port-failure channel before a success audit. Either redirect completion maps them to row-10 `exchange_failed`, emits no `identity.verify`, and never reaches the Bridge or `onIdentity`. Boundary tests cover one-byte and exact-2048-byte success controls, empty and whitespace-only strings, 2049 ASCII bytes, a multibyte overrun, and a getter that changes after its first read. One shared subject predicate serves the direct and redirect result snapshots, so a mutation that removes it makes every named path's test fail.
