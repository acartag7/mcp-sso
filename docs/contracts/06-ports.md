...
public low-level JWT signers independently require both their mint time and the
configured TTL offset to fit the canonical range; token-exchange owners also
...
description is the fixed `Identity rejected`.
