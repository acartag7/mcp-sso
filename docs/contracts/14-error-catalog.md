...
## Redirect and direct error channels

RFC 6749 §4.1.2.1 and §9.3 govern the error channel. `access_denied` from user denial, `unsupported_response_type`, `invalid_target`, `invalid_scope`, `invalid_request` from bad PKCE, and `server_error` use a 302 redirect to `redirect_uri?error=…` when they occur after `client_id` and `redirect_uri` validate. `invalid_redirect_uri`, a missing `client_id`, identity failure, `invalid_origin`, and consent-token integrity failures use a direct 4xx response.

§17.11 extends this rule for redirect-flow callbacks. In bridge completion, an identity rejection occurs after the signed flow context protects the validated `redirect_uri`, so the callback redirects with `access_denied`. Claims-only completion has no validated redirect target. It maps IdP denial and identity rejection to the same direct 400 `access_denied`, and maps exchange or completion failure to a direct 500. Flow-binding and integrity failures remain direct 4xx in both completions. These failures include a missing, invalid, expired, or replayed flow cookie, a state mismatch, and a missing code.
