...
| --- | --- |
| `CF_ACCESS_ISSUER`, `CF_ACCESS_AUDIENCE`, the Access certs URL | Cloudflare stack outputs |
...
- **Stack-provisioned provider credentials reach the process from stack outputs
  for the duration of the run.** Google's client credential is the explicit
  exception: it is created out of band and read from the owner-held mode-`0600`
  JSON file documented in `scripts/live/README.md`. Neither path commits,
  profiles, or echoes a credential — see the redaction rule in the receipts.
