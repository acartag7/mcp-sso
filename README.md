...
`RequestAuthorizer.authorize` fails closed. A missing token, a wrong audience, an expired token, or a missing scope throws an `OAuthError` whose `status` is 401 or 403. There is no local or unauthenticated mode.
