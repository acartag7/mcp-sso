...
2. Run `node session.mjs serve` in one terminal and `node session.mjs watch` in another. The first command starts all three legs and prints the client commands. The second records the audit-derived results under `.live-state/`. Stop both before starting another session. After a crash, run `node session.mjs cleanup` to attempt removal of the helper's six fixed Claude Code and Codex entries. For a signed-out Codex run that uses dynamic registration, add `--codex-dcr` to the watch command.
