...
# the shipped constructors, and only then executes the entry.
#
...
# Fail closed on the entry × leg pair: stack credentials are exported into an
# allowlisted script only, never into an arbitrary path.
case "$ENTRY:$LEG" in
  scripts/live/probe-cloudflare.mjs:cloudflare_access) ;;
  scripts/live/probe-entra.mjs:entra) ;;
  scripts/live/probe-google.mjs:google) ;;
  scripts/live/probe-e2e.mjs:cloudflare_access|scripts/live/probe-e2e.mjs:entra|scripts/live/probe-e2e.mjs:google) ;;
  examples/fastify-sqlite/index.ts:cloudflare_access|examples/fastify-sqlite/index.ts:entra|examples/fastify-sqlite/index.ts:google) ;;
  *) fail "unsupported entry/leg pair: $ENTRY $LEG" ;;
esac
[ -f "$REPO/$ENTRY" ] || fail "entry does not exist: $ENTRY"
if [ "$ENTRY" = "scripts/live/probe-e2e.mjs" ] && [ -z "${REDIS_URL:-}" ]; then
...
# A stale identity selector or OAuth override inherited from the invoking shell
# would either boot-fail the example (two selectors) or silently reshape the run
# (an allowlist mode, an insecure-localhost flag). Start from none of them.
for name in "${!ENTRA_@}" "${!CF_ACCESS_@}" "${!GOOGLE_@}" "${!OIDC_@}" "${!OAUTH_@}" "${!PROBE_@}"; do
  unset "$name"
done

# Stack reads. Every value is captured and validated; nothing is exported until
# the whole leg has been read, and no state is touched until it has passed.
output_raw() {
  local value
...
case "$LEG" in
  entra)
    ...
    export ENTRA_TENANT_ID ENTRA_CLIENT_ID ENTRA_CLIENT_SECRET ENTRA_REDIRECT_URI
    export ENTRA_UNMAPPED_GROUP ENTRA_GROUP_AUTHORIZATION_JSON
    ;;
  cloudflare_access)
...
export OAUTH_REDIRECT_ALLOWLIST
...
# Provider preflight over the assembled environment: exactly one selector, the
# example's own config parser, and the leg's shipped identity constructor. This
# runs BEFORE any prior state is cleared, so a bad stack output cannot cost the
# previous run's evidence.
support preflight "$LEG" || fail "stack outputs failed the provider preflight for leg $LEG"

if [ "$ENTRY" = "examples/fastify-sqlite/index.ts" ]; then
