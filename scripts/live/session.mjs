#!/usr/bin/env node
// Local helper for one operator running one manual client session.
...
import {
  ...
  readJson, resultFor, validateLegs, writeJson, writeResults,
...
function command(cmd, args) {
  return spawnSync(cmd, args, { encoding: "utf8", timeout: 15_000, maxBuffer: 64 * 1024 });
}
...
function modeFromEnvironment() {
  ...
  if (!["stored", "stateless"].includes(mode)) throw new Error("MCP_SSO_DCR_MODE must be stored or stateless");
  return mode;
}

function isAbsent(result) {
  return result.status === 1 && `${result.stdout ?? ""}\n${result.stderr ?? ""}`.includes("No MCP server named");
}
...
function validSession(value) {
  ...
  const fields = ["clean", "commit", "legs", "mode", "startedAt", "version"];
  ...
  if (value.version !== 1 || !["stored", "stateless"].includes(value.mode)
    || !/^[0-9a-f]{40,64}$/.test(value.commit) || typeof value.clean !== "boolean"
    || Number.isNaN(started) || new Date(started).toISOString() !== value.startedAt) return undefined;
...
async function serve(args) {
  ...
  const state = { version: 1, legs, mode, ...checkout(), startedAt: new Date().toISOString() };
  ...
  child.stdio[3].on("error", failReadiness);
  child.stdio[3].once("end", () => {
    if (readinessFailed || marker !== "ready\n") { failReadiness(); return; }
    try { writeJson(SESSION_FILE, state); ready = true; } catch { failReadiness(); }
  });
  let forwarded;
  ...
  if (!ready) rmSync(SESSION_FILE, { force: true });
  ...
  return clean && ready && !readinessFailed ? 0 : 1;
}
...
async function watch(args) {
  ...
  const session = validSession(readJson(SESSION_FILE));
  ...
  if (once && !failed) {
    writeResults(RESULTS_FILE, oneShot, { replace: true });
