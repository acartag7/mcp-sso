...
try {
  ...
  try {
    ...
    if (!ok("official SDK client completes a tool call on protected /mcp", false, "sdk_call_failed")) failures++;
  }
  ...
  for (const [name, value] of credentials) {
    if (typeof value !== "string" || value.length === 0) {
      if (!ok(`audit-leak check has the ${name} to inspect`, false, "credential_absent")) failures++;
      continue;
    }
    if (!ok(`audit never published the ${name}`, !all.includes(value))) failures++;
  }
  // Keep the signing credential out of the generic reporting helper entirely.
  // Its value is used only for the boolean comparison and can never become
  // output, even as a shortened prefix.
  const hasConsentSigningSecret = typeof process.env.OAUTH_CONSENT_SIGNING_SECRET === "string"
    && process.env.OAUTH_CONSENT_SIGNING_SECRET.length > 0;
  if (!hasConsentSigningSecret) {
    if (!ok("audit-leak check has the consent signing credential to inspect", false, "credential_absent")) failures++;
  } else if (all.includes(process.env.OAUTH_CONSENT_SIGNING_SECRET)) {
    if (!ok("audit never published the consent signing credential", false, "credential_leak_detected")) failures++;
  } else {
    ok("audit never published the consent signing credential", true);
  }
} finally {
