...
import {
  createEntraRedirectIdentity, entraIssuer, entraJwksUrl,
...
try {
  ...
  const rsaKeys = (jwksJson.keys ?? []).filter((key) => key.kty === "RSA");
  if (!ok("Entra JWKS serves RSA signing keys",
    jwks.status === 200 && rsaKeys.length > 0, `${rsaKeys.length} RSA keys`)) failures++;
  ...
  const groupAuthorization = JSON.parse(
    process.env.ENTRA_GROUP_AUTHORIZATION_JSON ?? "{}",
  );
  const groups = Object.keys(groupAuthorization.mapping ?? {});
  const normalizedGroups = new Set(groups.map((group) => group.toLowerCase()));
  if (!ok("boot accepted at least two mapped groups",
    groups.length >= 2, `${groups.length} groups mapped`)) failures++;
  if (!ok("every mapped key is a GUID",
    groups.length > 0 && groups.every((group) => guid.test(group)))) failures++;
  if (!ok("the deny-fixture GUID is absent from the mapping",
